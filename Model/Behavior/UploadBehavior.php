...
class UploadBehavior extends ModelBehavior
{
    ...
    function checkUploadSetup(&$model, $fieldData)
    {
        ...
        //check if this error is appearing correctly
    ...
    {
        if (empty($fieldData['filename']['tmp_name'])) {
            return true;
        }
        extract($this->settings[$model->name]);
    ...
    function _processUpload(&$model, $data = array(), $direct = false)
    {
        ...
        if (isset($info['extension'])) {
            ...
            $imageSizeInfo = @getimagesize($model->data[$model->name][$fileField]['tmp_name']);
            if (isset ($imageSizeInfo['mime'])) {
                $imageExtension = explode('/', $imageSizeInfo['mime']);
                $extension = $imageExtension[1];
            } else {
                $model->invalidate('filename', __('File doesn\'t have any extension. Please try with valid extension file.'));
                return false;
            }
        }
        ...
        //checking for the valid image file
        /*$validImage = $this->_checkForValidImage($model->data[$model->name][$fileField]['tmp_name']);
        if (!$validImage) {
            trigger_error(__('You are trying to upload invalid file. Please, try again.'), E_USER_WARNING);
            return false;
        }*/
        //$dir      = $this->_getPath($model, $dirFormat);
        //make directory if does not exists
		$filePathToStore = WWW_ROOT .'files/'.$dir;
		if (!file_exists($filePathToStore)) {
          mkdir($filePathToStore, 0777);
        }
        if (!$dir) {
		...
		$filenameToSave = $filename . '.' . $extension;
        ...
        if (file_exists($baseDir . $saveAs)) {
            if ($overwriteExisting) {
                ...
                $filenameToSave = $filename . '_' . $count . '.' . $extension;
            }
        }

		$imageSize = @getimagesize($model->data[$model->name][$fileField]['tmp_name']);
		...
		if (($imageSize != false) && ($imageSize[0] > 0 && $imageSize[1] > 0)) {
            ...
            //checking if the the size and dimensions are invalid
            if ($imageSize[0] != $imageSize[1]) {
                $model->invalidate('filename', __('File dimensions are not square, like 100x100.'));
                return false;
            }
            if ($model->data[$model->name][$fileField]['size'] >= (1024 * 1024)) {
                $model->invalidate('filename', __('File size should be less than 1mb. Please try again.'));
                return false;
            }
            if ($model->data[$model->name][$fileField]['size'] < (1024)) {
                $model->invalidate('filename', __('File size should be greater than 1kb. Please try again.'));
                return false;
            }
			// Change uploaded file's permissions
			chmod($baseDir . $saveAs, 0777);
			$data[$model->name]['avatar'] = $filenameToSave;
			$model->save($data);
			...
			$model->invalidate('filename', __('Trying to upload invalid file. Please try again.'));
			return false;
		}
    }//end _processUpload()
